...
class CodecsSuite
  ...
  /** Check that a value of an arbitrary type `A` can be encoded and then successfully decoded with the given pair of
    * encoder and decoder.
    */
  ...
  test("a 'String' value should be encoded as a JSON string (1pt)") {
    assertEquals(summon[Encoder[String]].encode("foo"), Json.Str("foo"))
  }
  ...
  test("a 'Boolean' value should be encoded as a JSON boolean (1pt)") {
    ...
    assertEquals(encoder.encode(true), Json.Bool(true))
    assertEquals(encoder.encode(false), Json.Bool(false))
  }
  ...
  test("a 'List[A]' value should be encoded as a JSON array (0pts)") {
    ...
    assertEquals(encoder.encode(xs), Json.Arr(List(Json.Num(1), Json.Num(2))))
  }
  ...
  test("A JSON array with at least one invalid element should be rejected (2pt)") {
  ...
  test("a 'Contacts' value should be encoded as a JSON object (1pt)") {
    ...
    assertEquals(encoder.encode(contacts), json)
  }
