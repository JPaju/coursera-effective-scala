...
class WikigraphSuite extends munit.FunSuite:
  ...
  def wikiResultGen[A](
    ...
    val eitherGen: Gen[Either[Seq[WikiError], A]] =
      ...
      if !withErrors then success
        ...
        Gen.containerOf[List, WikiError](errGen).flatMap { errors =>
    ...
    if !withFailure then success
      ...
      Gen.oneOf(true, false).flatMap { isFailed =>
  ...
  property("zip with domain errors") {
    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      if errs.nonEmpty then (errs == obtained.errors) :| "Errors are not accumulated correctly"
      else ((a.extractUnsafe, b.extractUnsafe) == obtained.extractUnsafe) :| "Result values are not zipped correctly"
    }
  }
  ...
  property("zip with domain errors and failures") {
    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      if a.failure.nonEmpty || b.failure.nonEmpty then
        sameErrorMessage(
          List(a, b).find(_.failure.isDefined).get.failure,
          obtained.failure
        ) :| "System failure is not reported correctly"
      else
        val errs: Seq[WikiError] = a.errors ++ b.errors
        if errs.nonEmpty then (errs == obtained.errors) :| "Errors are not accumulated correctly"
        else ((a.extractUnsafe, b.extractUnsafe) == obtained.extractUnsafe) :| "Results are not accumulated correctly"
    }
  }
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.nonEmpty then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| "map on a WikiResult failed with a system failure propagates the failure"
      ...
      else ((w.extractUnsafe + 1).toString == obtained.extractUnsafe) :| "map on successfull WikiResult is correct"
    }
  }
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.nonEmpty then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| "flatMap on a WikiResult failed with a system failure propagates the failure"
      else if w.errors.nonEmpty then
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.nonEmpty then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| "flatMap on a WikiResult failed with a system failure is correct"
      else if w.errors.nonEmpty then
        ...
        blockAndCompare(
          f(42),
          obtained
        ) :| "flatMap on a successfull WikiResult with a failing function produces a failed WikiResult with the correct domain error"
    }
  }
  ...
  property("flatMap with a function producing a system failure") {
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.isDefined then sameErrorMessage(w.failure, obtained.failure) :| "the old failure is not reported"
      else if w.errors.nonEmpty then
        (w.errors == obtained.errors) :| "the function is not invoked if the initial result contains errors"
      else sameErrorMessage(obtained.failure, Some(ex)) :| "the new failure is not reported"
    }
  }
  ...
  test("distanceMatrix") {
    ...
    val exp = WikiResult.successful[Seq[(String, String, Option[Int])]](
      Seq(("TestArticle-1", "TestArticle-3", Option(2)), ("TestArticle-3", "TestArticle-1", Option(1)))
    )
  ...
  property("namedLinks") {
    ...
    def setUp(withErrors: Boolean, withFailure: Boolean): Prop = {
      ...
      val failableLinks: Gen[WikiResult[Set[ArticleId]]] =
        val setOfGen =
          links.map(Gen.const).map(g => wikiResultGen(withErrors, withFailure)(using summon[Gen[WikiError]], g))
      ...
      val nameSearcher: Gen[ArticleId => WikiResult[String]] =
        ...
        val gens: List[Gen[(ArticleId, WikiResult[String])]] = all.map { (node: ArticleId) =>
          wikiResultGen(withErrors, withFailure)(using summon[Gen[WikiError]], Gen.const(s"TestArticle-${node.raw}"))
            .map(r => node -> r)
        }
        Gen.sequence(gens).map { listOfTuples => (art: ArticleId) =>
      ...
      forAll(generator) { (links, f) =>
        ...
        if links.failure.nonEmpty || links.errors.nonEmpty then
          ...
          // We need the first failure because failures are fail-fast
          ...
          if nameFail.nonEmpty then
            ...
            else (result.extractUnsafe == results.map(_.extractUnsafe)) :| "did not construct the correct set of names"
      }
    }
    val res: Prop =
      setUp(false, false) &&
  ...
  def checkProperty(prop: Prop): Unit =
    ...
    result.status match
      ...
      case org.scalacheck.Test.PropException(_, e, labels) =>
        failure(labels, s"An exception was thrown during property evaluation: $e")
