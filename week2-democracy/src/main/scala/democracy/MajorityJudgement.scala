...
/** A grade to assign to a candidate. There are seven possible grades (from the worst to the best):
  * `Bad`, `Mediocre`, `Inadequate`, `Passable`, `Good`, `VeryGood`, and `Excellent`.
  *
...
object Grade:
  /** @return
    ...
    *
    * The median grade can be computed by sorting the collection and taking the element in the
    * middle. If there are an even number of grades, any of the two grades that are just before of
    * after the middle of the sequence are correct median values.
    *
    * Grades can be compared by using their `ordinal` method.
    *
...
/** An election is defined by a simple description and a set of possible candidates.
  * @param description
  *   Description of the election (e.g., “Presidential Election”)
  * @param candidates
  *   Possible candidates
  */
case class Election(description: String, candidates: Set[Candidate]):
  /** @return
    *   The candidate that wins this election, according to the Majority Judgement voting process.
    *
    * @param ballots
    *   The ballots for this election
    *
    * The ballots ''must'' assign a grade to each of the `candidates` of this election.
    */
  ...
  /** @return
    *   The winner of this election, according to the Majority Judgement voting process.
    *
    * @param gradesPerCandidate
    *   The grades that have been assigned to each candidate by the voters.
    */
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] =
        gradesPerCandidate
          .filter((_, grades) => Grade.median(grades).ordinal == bestMedianGrade.ordinal)
