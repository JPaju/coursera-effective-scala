...
/** This suite aggregates the tests for the InMemoryModel and the PersistentModel. It is used by the
    ...
    actual match
      ...
      case Some(task) =>
        assertEquals(
          task,
          expected,
          s"Using $modelName: We expected the task $expected but we received $task instead."
        )
  ...
  def assertTaskActive(
    ...
    task match
      ...
      case Some(t) =>
        assert(
          t.state.active,
          s"Using $modelName: We expected the task's state to be active but it was ${t.state}"
        )
  ...
  def assertTaskCompleted(
    ...
    task match
      ...
      case Some(t) =>
        assert(
          t.state.completed,
          s"Using $modelName: We expected the task's state to be completed but it was ${t.state}"
        )

  // The tests, parameterized by a fixture

  def allTests(fixture: FunFixture[(String, Model)]): Unit =
    fixture.test("Created tasks can be read") {
      case (name, model) =>
    ...
    fixture.test("Updated tasks can be read") {
      case (name, model) =>
    ...
    fixture.test("Deleted tasks are no longer read") {
      case (name, model) =>
        val id1 = model.create(task1)
        val id2 = model.create(task2)
    ...
    fixture.test("Tasks returns all inserted tasks in insertion order") {
      case (name, model) =>
        val id1 = model.create(task1)
        val t1  = model.tasks
    ...
    fixture.test("Tasks(tag) returns only tasks with given tag") {
      case (name, model) =>
        val id1 = model.create(task1)
        val id2 = model.create(task2)
    ...
    fixture.test("complete changes state to completed, if task is not already completed") {
      case (name, model) =>
        val id1 = model.create(task1)
        val id2 = model.create(task2)
    ...
    fixture.test("tags returns all tags") {
      case (name, model) =>
        val id1 = model.create(task1)
