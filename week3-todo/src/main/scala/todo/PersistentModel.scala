...
/** The PersistentModel is a model that saves all data to files, meaning that tasks persist between
  * restarts.
  *
...
object PersistentModel extends Model:
  ...
  /** Load Tasks from a file. Return an empty task list if the file does not exist, and throws an
    * exception if decoding the file fails.
    */
  def loadTasks(): Tasks =
    if Files.exists(tasksPath) then load[Tasks](tasksPath)
    else Tasks.empty

  /** Load an Id from a file. Returns Id(0) if the file does not exist, and throws an exception if
    * decoding the file fails.
    */
  def loadId(): Id =
  ...
  /** Load JSON-encoded data from a file.
    *
    * Given a file name, load JSON data from that file, and decode it into the type A. Throws an
    * exception on failure.
    *
    * It is not necessary to use this method. You should be able to use loadTasks and loadId
  ...
  /** Save data to a file in JSON format.
    *
    * Given a file name and some data, saves that data to the file in JSON format. If the file
    * already exists it is overwritten.
    *
    * It is not necessary to use this method. You should be able to use saveTasks and saveId
    * instead, which have a simpler interface.
    */
  ...
  def create(task: Task): Id =
    ...
    val newId    = currentId.next
    val newTasks = Tasks(currentTasks.toList :+ (newId, task))
    ...
    read(id)
      ...
      .map { updatedTask =>
        val updatedTasks = tasks
          .toList
          .map {
            case (taskId, _) if taskId == id => (id, updatedTask)
            case entry                       => entry
          }
  ...
  def delete(id: Id): Boolean =
    val allTasks = loadTasks().toMap

    if allTasks.contains(id) then
      val newTasks = Tasks(allTasks.removed(id))
      saveTasks(newTasks)
      true
    else false
